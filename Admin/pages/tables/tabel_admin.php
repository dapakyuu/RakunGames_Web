...
if (isset($_POST['submit'])) {
    ...
    // Cek username/email duplikat di tabel admin (kecuali data sendiri)
    $sql = "SELECT * FROM admin WHERE (username=? OR email=?) AND id_admin != ?";
    $stmt = $koneksi->prepare($sql);
    ...
    if ($password) {
        $query = "UPDATE admin SET username = ?, email = ?, phone = ?, pass = ? WHERE id_admin = ?";
        $stmt = $koneksi->prepare($query);
        $stmt->bind_param("ssssi", $username, $email, $phone, $password, $id);
    } else {
        $query = "UPDATE admin SET username = ?, email = ?, phone = ? WHERE id_admin = ?";
        $stmt = $koneksi->prepare($query);
...
<head>
    ...
    <title>AdminRakunGames | Tabel Admin</title>
                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <?php if ($_SESSION["is_admin"]): ?>
                            ...
                            <li class="nav-item">
                                ...
                                <ul class="nav nav-treeview">
                                    <li class="nav-item">
                                        <a href="tabel_admin.php" class="nav-link active">
                                            <i class="far fa-circle nav-icon"></i>
                                            <p>Admin</p>
                                        </a>
                                    </li>
                                                ...
                                                echo "
                                                    <tr class='text-center'>
                                                        ...
                                                        <td class='project-actions text-center'>
                                                            ...
                                                            <form method='POST' action='tabel_admin.php' style='display:inline;'>
                                                                <input type='hidden' name='delete_id' value='$row[id_admin]'>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="editForm" action="tabel_admin.php" method="POST">
